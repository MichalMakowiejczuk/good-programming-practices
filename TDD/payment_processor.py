import logging
from payment_gateway import PaymentException, NetworkException, RefundException, TransactionStatus, TransactionResult, PaymentGateway
...
class PaymentProcessor:
    ...
    def process_payment(self, user_id: str, amount: float) -> TransactionResult:
        ...
        try:
            ...
            if result.success:
                logger.info(f"Payment successful for user: {user_id} with transaction ID: {result.transaction_id}")
            else:
                logger.warning(f"Payment failed for user: {user_id}. Reason: {result.message}")
            return result
    ...
    def refund_payment(self, transaction_id: str) -> TransactionResult:
        ...
        try:
            ...
            if result.success:
                ...
                logger.warning(f"Refund failed for transaction ID {transaction_id}. Reason: {result.message}")
            return result
        except (NetworkException, RefundException) as e:
            logger.error(f"Refund processing error for transaction ID {transaction_id}: {str(e)}")
    ...
    def get_payment_status(self, transaction_id: str) -> TransactionStatus:
        ...
        try:
            ...
            logger.error(f"Failed to retrieve status for transaction ID {transaction_id} due to network error.")
